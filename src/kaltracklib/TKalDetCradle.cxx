                           ...
                           std::unique_ptr<TVTrack> &help)  // pointer to update track object
{
  ...
  std::cout << "Index of site from: " << fridx << "\t Index of site to: " << toidx << std::endl;
  ...
  //const TVMeasLayer &ml_from = from.GetHit().GetMeasLayer();
  //const TVSurface *sfp_from = dynamic_cast<const TVSurface *>(&ml_from); //surface at current
  ...
  std::cout << "[TKalDetCradle] Running CalcXingPointWith" << std::endl;
  sfp->CalcXingPointWith(hel, xto, fito, 0, eps);
  std::cout << "[TKalDetCradle] xto (x,y,z) = (" << xto.X() << ", " << xto.Y() << ", " << xto.Z() << ")" << std::endl;
  ...
  for (Int_t ito=fridx; (di>0 && ito<=toidx)||(di<0 && ito>=toidx); ito += di) {
	...
	ml.CalcQms(isout, hel, fid, Qms); // Qms for this step, using the fact that the material was found to be outgoing or incomming above, and the distance from the last layer
      ...
      DF.DebugPrint(std::cout, "DF");
      Qms.DebugPrint(std::cout, "Qms");
